...
import { MediaKind, RejectReason, type DetectedFile } from '@/features/media/types/media';
import { photoProblem, readPhotoHeader } from '@/features/media/utils/image';
import { readHead, unsupportedFormat } from '@/features/media/utils/media';
...
const HEIF_AVIF_BRANDS = new Set([
  'heic',
  'heix',
  'heim',
  'heis',
...
export class MediaDetector {
  ...
  async detect(): Promise<DetectedFile> {
    ...
    if (brand && HEIF_AVIF_BRANDS.has(brand)) {
  ...
  private unsupported(): DetectedFile {
    return this.refused(unsupportedFormat(this.kind));
  }
